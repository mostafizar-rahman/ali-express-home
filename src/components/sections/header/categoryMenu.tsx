...
const CategoryMenu = () => {
    ...
    return (
        <div className="relative w-[256px]">
            <h2 className="bg-[#f5f5f5]  inline-flex items-center justify-between gap-2 rounded-full py-2 px-6 cursor-pointer w-full">
            ...
            <ul className="bg-[#f5f5f5] w-full max-h-[523px] box-border absolute">
                {categoryList.map((category) => (
                    <li key={category.url || category.name} className="relative full group">
                        <a href={category.url || '#'} className=" text-sm  flex items-center gap-2 py-3 pl-6 pr-3 hover:bg-white hover:font-bold transition-all">
